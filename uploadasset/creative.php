...
<img style="height: 15px;position: absolute;top: 255px;left: 3px;" src="https://hcurvecdn.com/atest/pooja/updateanimation/progressbarselection.png"></div>
 ...
 <div class="page_info">Click to <a target="_blank" href="https://docs.google.com/document/d/1pXdBxKzl-cJYZbatCwG5jpQOt2Kvku5UiijdHQHWg_0">Know More</a></div>
        ...
        $.ajax({
            ...
            success : function(dt){
                ...
                for(var x1 in dt.table2){
                    if(dt.table2[x1] != null){
                    cm += `<li class="main_nav" id="${x1}">
                             <div class="menu_title" onclick="clicks(event);">
                                ${x1}
                                <img id="arrow_icon" src="https://hcurvecdn.com/hoichoi/firstcampaign/300x250/DC7_F1_F2/down_arrow.png" alt="icon" />
                                </div>
                                <ul class="sub_menu">
                                <li>${dt.table2[x1]}</li>
                                </ul>
                ...
                var number_of_city_div = document.querySelectorAll(".city_dv").length;
                ...
                if(number_of_city_div > 1){
                    for(var xb=2;xb<=number_of_city_div;xb++){
                    city_alignment_input(xb); 
                    city_alignment_script(xb)
                    }
                }
                ...
                dim_check(dt.dims);
                if(geo_city != ""){
                        document.getElementById("btn_clone").innerHTML += `<button id="clone_city">Add city</button>`
                    if(document.querySelector("#city_dv") == null){
                        var divss = "<div id='city_dv' class='city_dv' style='position:absolute;top:0;left:0;'><span id='city_name' style='text-transform:capitalize'>"+dt.default_city+"</span></div>";
                        document.querySelector("#clickurl").innerHTML += divss;
                    }
                    document.getElementById("clone_city").addEventListener("click",(e)=>{
                    number_of_city_div = document.querySelectorAll(".city_dv").length;
                        var city_div = document.getElementById("city_dv");
                        var clone_city_div = city_div.cloneNode(true);
                        clone_city_div.id = city_div.id+(Number(number_of_city_div)+1);
                        clone_city_div.children[0].id = clone_city_div.children[0].id+(Number(number_of_city_div)+1);
                        document.getElementById("clickurl").appendChild(clone_city_div);
                        city_alignment_input(Number(number_of_city_div)+1)
                        city_alignment_script(Number(number_of_city_div)+1)
                    })
                ...
                if(crt_date == 'true'){
                    ...
                    if(document.querySelector("#date_dv") == null){
                            var div_date = "<div id='date_dv' class='date_cls' style='position:absolute;top:0px;left:0;'><span id='date_nm' style='text-transform:capitalize;font-size:12px;'></span><input type='hidden' id='dt_hd' value='date hmon' /></div>";
                            ...
                            if(current_hrs == 0){
                                format_12hrs = 0
                            }else if(current_hrs > 12){
                                format_12hrs = current_hrs - 12
                            }else{
                                format_12hrs = current_hrs
                            }
                            document.getElementById("date_nm").innerText = "";
                            document.getElementById("date_nm").innerText = document.getElementById("dt_hd").value.replace("secs",current_secs).replace("mins",current_mns).replace("24hr",current_hrs).replace("12hr",format_12hrs).replace("ampm",am_pm).replace("date",current_date).replace("fday",current_day).replace("hday",current_day_short).replace("fmon",current_month).replace("hmon",current_month_short).replace("nmon",month_number).replace("fyear",current_year).replace("hyear",short_year);
                            `;
                            ...
                            if(number_of_date_div > 0){
                               ...
                               for(var dtx=0;dtx<clone_date_div.children.length;dtx++){
                                ...
                                var innerScript_clone = `
                                                    ...
                                                    document.getElementById("date_nm${Number(number_of_date_div)+1}").innerText = document.getElementById("dt_hd${Number(number_of_date_div)+1}").value.replace("secs",current_secs).replace("mins",current_mns).replace("24hr",current_hrs).replace("12hr",format_12hrs).replace("ampm",am_pm).replace("date",current_date).replace("fday",current_day).replace("hday",current_day_short).replace("fmon",current_month).replace("hmon",current_month_short).replace("nmon",month_number).replace("fyear",current_year).replace("hyear",short_year);
                                    `;
                ...
                // city alignment input field cloning
                function city_alignment_input(count){
                    var city_align_clone = document.querySelector("#city_data").cloneNode(true);
                        city_align_clone.id = `${city_align_clone.id}${count}`;
                        city_align_clone.children[1].children[0].querySelectorAll("input").forEach(el =>{
                            el.id = el.id+count;
                        });
                        document.querySelector(".nav-links").appendChild(city_align_clone)
                }
                
                // date alignment input field cloning
                function alignment_input(count){
                    var date_align_clone = document.querySelector("#current_date").cloneNode(true);
                ...
                function alignment_script(count){
                    var date_align = `document.getElementById("date_format${count}").addEventListener("keyup",(e)=>{
                                ...
                                document.getElementById("date_nm${count}").innerText = document.getElementById("dt_hd${count}").value.replace("secs",current_secs).replace("mins",current_mns).replace("24hr",current_hrs).replace("12hr",format_12hrs).replace("ampm",am_pm).replace("date",current_date).replace("fday",current_day).replace("hday",current_day_short).replace("fmon",current_month).replace("hmon",current_month_short).replace("nmon",month_number).replace("fyear",current_year).replace("hyear",short_year);
                            });
                    ...
                    // city alignment srcipt code
                function city_alignment_script(count){
                    var city_align = `document.getElementById("city_top${count}").value = Number(document.getElementById("city_dv${count}").style.top.replace(/px/g, ''));
                        document.getElementById("city_left${count}").value = Number(document.getElementById("city_dv${count}").style.left.replace(/px/g, ''));
                        document.getElementById("city_size${count}").value = Number(document.getElementById("city_name${count}").style.fontSize.replace(/px/g, ''));
                        document.getElementById("city_transform${count}").value = document.getElementById("city_name${count}").style.textTransform;
                        document.getElementById("city_width${count}").value = Number(document.getElementById("city_dv${count}").style.width.replace(/px/g, ''));
                        document.getElementById("city_align${count}").value = document.getElementById("city_dv${count}").style.textAlign;
                        document.getElementById("city_zindex${count}").value = Number(document.getElementById("city_dv${count}").style.zIndex);
                        document.getElementById("city_family${count}").value = document.getElementById("city_dv${count}").style.fontFamily;
    
                        document.getElementById("city_top${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.top = e.target.value+"px";
                        });
                        document.getElementById("city_left${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.left = e.target.value+"px";
                        });
                        document.getElementById("city_color${count}").addEventListener("input",(e)=>{
                            document.getElementById("city_name${count}").style.color = e.target.value;
                        });
                        document.getElementById("city_size${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_name${count}").style.fontSize = e.target.value+"px";
                        });
                        document.getElementById("city_transform${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_name${count}").style.textTransform = e.target.value;
                        });
                        document.getElementById("city_width${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.width = e.target.value+"px";
                        });
                        document.getElementById("city_align${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.textAlign = e.target.value;
                        });
                        document.getElementById("city_zindex${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.zIndex = e.target.value;
                        });
                        document.getElementById("city_family${count}").addEventListener("keyup",(e)=>{
                            document.getElementById("city_dv${count}").style.fontFamily = e.target.value;
                        });`
                        
                        // Creating script tag for date alignment script
                        var city_script_doc = document.createElement("script");
                        var innerScriptTextdoc = document.createTextNode(city_align)
                        city_script_doc.appendChild(innerScriptTextdoc)
                        
                        // appending to body tag
                        $("body").append(city_script_doc)
                }
    ...
    $(document).on("click","#sv_nxt",function(e){
        ...
        if(asset_script != undefined && asset_script != ""){
            if(asset_status){
                ...
                if(count == dim_count){
                var nav_links = document.querySelectorAll(".main_nav");
                nav_links.forEach(el => {
                    el.children[1].children[0].querySelectorAll("input").forEach(el =>{
                        align_obj[`${el.id}`] = el.value
                    })
                    align_obj_outer[`${el.id}`]=align_obj
                    align_obj = {};
                    align_arr.push(align_obj_outer)
                    align_obj_outer = {}
                })
                    $("#sv_nxt").text("Proceed");
                    $("#pro_msg").text("Click the Proceed button to continue");
