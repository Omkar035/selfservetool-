                ...
                if(mysqli_num_rows($data_creatcode)>0){
                    while($row_creatcode=mysqli_fetch_assoc($data_creatcode)){
                        ...
                        if($row_creatcode['finalcode']!=""){
                            $finalcode=$content."<script id=script".$single_dim.">".$row_creatcode['animation']."</script>";
                    ...
                    $finallycode=$content."<script id=script".$single_dim.">".$defaultanimation."</script>";
                    $sql_insert = "INSERT INTO creativecode (username,name,campaign,type,cdata,client,dimension,filter,status,content,finalcode,asset_used,test_anim,animation) VALUES ('$username','$temp','$camp','static','popular','$client','$single_dim','$fcat','active','$content','$finallycode','$asset_r','$defaultanimation','$defaultanimation')";
                    ...
                    if($data_insert){
                        ...
                        $resp = array("status"=>true,"message"=>$data_insert);
                        echo json_encode($resp);
                    }
                }
    }}
