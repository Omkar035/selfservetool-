...
if(isset($_POST['upload'])){
    header("location: https://publisherplex.io/selfserve/uploadasset/uploadimage.php?id=$id");
}
        ...
        $dat2=mysqli_query($connectDB,$update); 
           ...
           if(isset($_POST['submit'])){
            ...
            if(mysqli_num_rows($data33)>0){
                ...
                $sql43="SELECT * FROM `templates` WHERE template_name='$tmp_name' AND dim='$dim'";
            $data43=mysqli_query($connectDB,$sql43);
            if(mysqli_num_rows($data43)>0){
                while($row43=mysqli_fetch_assoc($data43)){
                    $asset_used=$row43['assets_req'];
                    ?> <script> alert(<?php echo $asset_used  ?>)</script> <?php
                }}
                $sql1 = "INSERT INTO creativecode (name,campaign,type,cdata,client,dimension,filter,status,content,clicks,impressions,asset_used) VALUES ('$tmp_name','$camp33','static','popular','$client33','$dim','$autofcat','active','$htmlcode','$landing_url','$imperession_trk','$asset_used')";
                $data1=mysqli_query($connectDB,$sql1); 
