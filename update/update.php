...
<ul class="list-unstyled multi-steps" id="mobile_resp">    
...
<img style="height: 15px;position: absolute;top: 332px;left: 3px;" class="probardot" src="https://hcurvecdn.com/atest/pooja/updateanimation/progressbarselection.png"></div>
    ...
    <h1 style="margin-left:20px;color:black;font-weight:bolder;font-size:3em;font-family:bold;margin-left:340px;position:absolute;top:93px;">Update <span style="color:#1c82e3;font-weight:bolder">Animations</span></h1><img style="width:25px;position: absolute;left:52%;top:17%;" src="https://publisherplex.io/selfserve/info.png" class="img3" id="feedifo"/><span class="infotxt3"><li>Click on update aniamtion button for adding required animation to our creative.</li><li>For adding trackers to creative click on add trackers button.</li></span>
...
if(mysqli_num_rows($data1)>0){
 ...
 <tr>
     ...
     $code=$rowa['content'];
      ...
      $code=$rowa['finalcode'];} ?>
      <div style="position:relative;left:325px;top:250px;margin-bottom:50px;width:<?php echo $abc2[0]+300 ?>px;height:<?php echo $abc2[1]+20 ?>px">
 ...
 <div id="dim<?php echo $f; ?>" style="text-align:center;display:none"><?php echo $rowa['dimension']; ?></div>
 <!--<td style="text-align:center;"><a href="./testanim.php?id=<?php echo $id2 ?>">Update Animation</a></td></tr>-->
  <div style="border:none;position:absolute;top:0px;left:<?php echo $abc2[0]+160 ?>px;"> <form method="POST"><button id="btn1" class="cta btn--shimmer" name="log-<?php echo $id2 ?>";>Update Animation</button></form></div>
  </div>

  <?php    
  
  if(isset($_POST['log-'.$id2])){
...
$fcat=$_SESSION['fcat'];
                $log="INSERT INTO `log_file`(`client_name`, `email`, `adtag` ,`fcat`, `date_time`, `event`) VALUES ('$client','$email','$adtag_type','$fcat','$time','Update animation clicked')";
                $result_log=mysqli_query($connectDB,$log);
   header("location:  ./testanim.php?id=$id2");
...
while($rowreq2=mysqli_fetch_assoc($datareq2)){
 ...
 header("Refresh:0");
        ...
        if($adtag_type=="dcm" || $_SESSION['lineitem']=="true"){
            ?>

            <form method="POST">
                <button type="submit" class="btn btn-success" name="create" style="position: absolute;top:46px;right: -200px;font-size: 19px;font-weight: bold;background: #1379d6;border: none;width: 155px;height: 40px;border-radius:10px">Click Test</button>
            ...
            ?>
                <form method="POST"><button name="log-adtracker" class="btn btn-success" style="position: absolute;top:46px;right: -200px;font-size: 19px;font-weight: bold;background: #1379d6;border: none;width: 155px;height: 40px;border-radius:10px">Add Trackers</button></form>
...
$client=$_SESSION['client_name'];
$fcat=$_SESSION['fcat'];
                $log="INSERT INTO `log_file`(`client_name`, `email`, `adtag` ,`fcat`, `date_time`, `event`) VALUES ('$client','$email','$adtag_type','$fcat','$time','Add tracker clicked')";
                $result_log=mysqli_query($connectDB,$log);
...
<label class="logoutLblPos">
 ...
 <div class="page_info">Click to <a target="_blank" href="https://docs.google.com/document/d/1jkCP0uCjt0d937r41MBqgbAly5NKvvLsxsvh76UGjhg">Know More</a></div>
...
$client=$_SESSION['client_name'];
$fcat = $_SESSION['fcat'];
                $log="INSERT INTO `log_file`(`client_name`, `email`, `date_time`, `event`,`fcat`,`adtag`) VALUES ('$client','$email','$time','publish ad','$fcat','dcm')";
                $result_log=mysqli_query($connectDB,$log);
