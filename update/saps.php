...
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.1.0-beta.1/css/select2.min.css" integrity="sha512-BUP3qnv/dyJWgO8nyvkrXk91V7RPTZczwW8NYISvWsADoMFqMfsy43bA+8VvI9dQa7/GDZ1EuRGn8YrF1rBpbA==" crossorigin="anonymous" />
<script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.1.0-beta.1/js/select2.min.js" integrity="sha512-pN4+j9HxjsZtbwm0ZKjPHcqJTxUzth6UQMQOybhTAdF+mTmB9Gq3qTehZMKYHwEyfDITpB1GICvdA34pz2QHlQ==" crossorigin="anonymous"></script>
...
<!--<form id="formId" method="post"><input type="text" name="client" id="txtCountry" class="typeahead" autocomplete="off" placeholder="Search animation"/></form> -->
         ...
         if(mysqli_num_rows($data2)>0){
            //  <input type="text" name="client" id="txtCountry" class="typeahead" autocomplete="off" placeholder="Search client"/>
                        while($row2=mysqli_fetch_assoc($data2)){
                            ...
                            ?> <option value='<?php echo $row2["anim"] ?>'><?php echo $row2['name'] ?><input type="text" name="client" id="txtCountry" class="typeahead" autocomplete="off" placeholder="Search client"/></option> <?php
                        }}
                                  ?>
                                  </select>
